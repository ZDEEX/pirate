...
const std::string ZDEEXOceanGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ZDEEXOceanGUI::DEFAULT_WALLET = "~Default";

ZDEEXOceanGUI::ZDEEXOceanGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void ZDEEXOceanGUI::createActions()
{
    ...
    //sendCoinsAction->setStatusTip(tr("Send coins to a ZDEEX address"));
    ...
    //Note: zdeexoceangui() gets run before the configuration file is scanned.
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your ZDEEX transparent addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified ZDEEX transparent addresses"));
    ...
    openAction->setStatusTip(tr("Open a zdeex: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible ZDEEX command-line options").arg(tr(PACKAGE_NAME)));
...
void ZDEEXOceanGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void ZDEEXOceanGUI::createToolBars()
{
    if(walletFrame)
    {
...
void ZDEEXOceanGUI::setColdStorageLayout( )
{
    QSettings settings;
    bool fEnableZSigning = settings.value("fEnableZSigning").toBool();
    if (fEnableZSigning==true) 
...
void ZDEEXOceanGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ZDEEXOceanGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ZDEEXOceanGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ZDEEXOceanGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ZDEEXOceanGUI::setWalletActionsEnabled(bool enabled)
{
...
void ZDEEXOceanGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void ZDEEXOceanGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &ZDEEXOceanGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void ZDEEXOceanGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void ZDEEXOceanGUI::macosDockIconActivated()
{
    show();
    activateWindow();
}
...
void ZDEEXOceanGUI::optionsClicked()
{
...
void ZDEEXOceanGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void ZDEEXOceanGUI::showDebugWindow()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::showDebugWindowActivateConsole()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::showHelpMessageClicked()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
#ifdef ENABLE_WALLET
void ZDEEXOceanGUI::openClicked()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::resetUnlockTimer() {
    if (walletFrame) walletFrame->resetUnlockTimer();
}

void ZDEEXOceanGUI::gotoOverviewPage()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::gotoHistoryPage()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::gotoReceiveCoinsPage()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::gotoZSendCoinsPage(QString addr)
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::gotoZSignPage()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::gotoImportSK()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::gotoImportVK()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::showSeedPhrase()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ZDEEXOceanGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to ZDEEX network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void ZDEEXOceanGUI::setNumConnections(int count)
{
    (void)count;
    updateNetworkState();
}

void ZDEEXOceanGUI::setNetworkActive(bool networkActive)
{
    (void)networkActive;
    updateNetworkState();
}

void ZDEEXOceanGUI::updateHeadersSyncProgressLabel()
{
...
void ZDEEXOceanGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ZDEEXOceanGUI::message(const QString &title, const QString &message, unsigned int style, int *ret)
{
    QString strTitle = tr("ZDEEX"); // default title
...
void ZDEEXOceanGUI::changeEvent(QEvent *e)
{
...
void ZDEEXOceanGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ZDEEXOceanGUI::showEvent(QShowEvent *event)
{
    (void)event;
...
#ifdef ENABLE_WALLET
void ZDEEXOceanGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
...
void ZDEEXOceanGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ZDEEXOceanGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ZDEEXOceanGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
/*
bool ZDEEXOceanGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
*/
void ZDEEXOceanGUI::setHDStatus(int hdEnabled)
{
...
void ZDEEXOceanGUI::setEncryptionStatus(int status)
{
    //Check Settings
    QSettings settings;
    bool fEnableZSigning = settings.value("fEnableZSigning").toBool();
...
void ZDEEXOceanGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ZDEEXOceanGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ZDEEXOceanGUI::detectShutdown()
{
...
void ZDEEXOceanGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ZDEEXOceanGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

void ZDEEXOceanGUI::ShowProgress(QString title, int nProgress)
{
    if (nProgress == 0)
    {
...
// Default: BTN_CANCEL
static int ThreadSafeMessageBox(ZDEEXOceanGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ZDEEXOceanGUI::subscribeToCoreSignals()
{
...
void ZDEEXOceanGUI::unsubscribeFromCoreSignals()
{
...
void ZDEEXOceanGUI::toggleNetworkActive()
{
