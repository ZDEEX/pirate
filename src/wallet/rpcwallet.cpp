...
UniValue encryptwallet(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (!pwalletMain->EncryptWallet(strWalletPass)) {
        ...
        //shutdown
        StartShutdown();
        return "wallet encryption failed; ZDEEX server stopping, restart to restore unencrypted wallet.";
    } else {
...
UniValue z_sign_offline(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp==false)
    {
      ...
      if (sProject != "arrr")
      {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Only ZDEEX Chain (arrr) supported");
      }
    ...
    if (fHelp || bParamSizeInvalid==true) //This test will be adapted if multiple versions gets implemented
      throw runtime_error(
            ...
            "\nArguments:\n"
            "0. Project                 (string, required) Project: arrr=ZDEEX Chain\n"
