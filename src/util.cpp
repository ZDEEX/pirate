...
std::string LicenseInfo()
{
    return "\n" +
           ...
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The ZDEEX developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure ZDEEX Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("ZDEEX developers") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The ZDEEX developers";
    }
